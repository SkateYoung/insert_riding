...
if __name__ == "__main__":
    ...
    # 步骤 1：启动数字路网引擎模型实例
    city = CityGraph("dxc_traffic_shp/dxc_rule.shp") # 必要初始化操作
    
    # 步骤 2：分配运营配置、声明出勤车组队 (增至三台以适配三车指派中心)  # 必要初始化操作
    fleet = [
    ...
    # 注入业务元数据 (对应 3 个固定司机与车辆信息)      # 必要初始化操作
    ...
    test_orders = [(113.395, 23.045, 113.410, 23.055)]
    ...
    # print("[OK] 后端元数据与图论底座导出完成 (map_data.js)。")
    # 步骤 6：启动后台持续匹配引擎 (采用独立守护线程，不阻塞主程序)     # 必要初始化操作
    ...
    # 步骤 7：保持主程序运行，以便后续接入强化学习或其他独立任务
    try:
        print("[系统] 主线程已就绪，按 Ctrl+C 可安全终止进程。")
        while True:
            # 您可以在此主线程循环内做任何其他事情，如持续生成订单、触发 RL Agent 推演等
            time.sleep(5)
            for v in fleet:
                print(v.id + ":")
                print(v.planned_route)
                print("\n")
